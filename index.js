...
function createDictionary(entries) {
  ...
  entries.forEach(([key, value]) => {
    dictionary.set(key, value); // Додавання пари ключ-значення до словника
  });

  return dictionary; // Повертаємо створений словник (Map)
}
...
function convertMapToObject(map) {
   const obj = {}; // Створення порожнього об'єкта

  map.forEach((value, key) => {
    obj[key] = value; // Додавання ключа та значення до об'єкта
  });
...
function setValue(dictionary, key, value) {
 if (dictionary.has(key)) {
    // Ключ вже існує, оновлюємо його значення
    dictionary.set(key, value.toUpperCase());
  } else {
    // Ключ не існує, додаємо нову пару ключ-значення
...
if (dictionary.has(key)) {
    // Ключ існує, видаляємо його та відповідне значення
    dictionary.delete(key);
    return true;
  } else {
    // Ключ не знайдено, повертаємо false
    return false;
  }
}
...
function getKeysStartingWith(dictionary, letter) {
  ...
  for (const key of dictionary.keys()) {
    ...
    if (key.startsWith(letter)) {
      ...
      keysStartingWithLetter.push(key); // Додаємо ключ до масиву
    }
  }

  return keysStartingWithLetter; // Повертаємо масив ключів
...
function addKeyValuePairs(dictionary, entries) {
   let added = 0; // Змінна для збереження кількості доданих ключів
  let rejected = 0; // Змінна для збереження кількості відхилених ключів
  ...
  entries.forEach(([key, value]) => {
    ...
    if (!dictionary.has(key)) {
      ...
      dictionary.set(key, value); // Додаємо ключ-значення до словника за допомогою методу set
      added++; // Збільшуємо лічильник доданих ключів на 1
    } else {
      rejected++; // Збільшуємо лічильник відхилених ключів на 1
    }
  });

  return { dictionary, added, rejected }; // Повертаємо об'єкт зі зміненим словником, кількістю доданих та відхилених ключів
}
...
function transformDictionary(dictionary) {
 const keys = []; // Масив для збереження ключів
  ...
  const entries = []; // Масив для збереження пар ключ-значення

  for (const key of dictionary.keys()) {
    // Цикл for...of і метод keys для перебору ключів словника
    keys.push(key); // Додавання ключа до масиву keys
  }

  for (const value of dictionary.values()) {
    // Цикл for...of і метод values для перебору значень словника
    values.push(value); // Додавання значення до масиву values
  }

  for (const entry of dictionary.entries()) {
    // Цикл for...of і метод entries для перебору пар ключ-значення словника
    entries.push(entry); // Додавання пари ключ-значення до масиву entries
  }
...
function checkPresence(dictionary, keys) {
   const presence = []; // Масив для збереження результатів перевірки

  keys.forEach((key) => {
    // Цикл forEach для перебору масиву ключів
    presence.push(dictionary.has(key)); // Додавання результату перевірки до масиву presence
  });
...
function getFilteredDictionarySize(dictionary, filter) {
  const filteredDictionary = new Map(); // Новий Map об'єкт для зберігання елементів, що відповідають фільтру

  for (const [key, value] of dictionary.entries()) {
    // Цикл for...of разом з методом entries() для перебору пар [ключ, значення] словника
    if (filter(key, value)) {
      filteredDictionary.set(key, value); // Додавання пари [ключ, значення] до фільтрованого словника
    }
  }
...
function sortByValues(dictionary) {
   const sortedArray = [...dictionary.entries()].sort((a, b) => {
  ...
  const sortedDictionary = new Map(sortedArray); // Конвертуємо відсортований масив пар ключ-значення назад у словник
...
function resetDictionary(dictionary, maxSize) {
 if (dictionary.size > maxSize) {
    dictionary.clear(); // Очищуємо словник за допомогою методу clear
 ...
 const transformedSet = new Set();
  ...
  for (const key of dictionary.keys()) {
    if (typeof key === "string") {
      transformedSet.add(key.toUpperCase());
    } else if (typeof key === "number") {
      transformedSet.add(key + 1);
    }
  }
